...
cleanup() {
  ...
  if [ -n "${CREATED_IMAGE_ID:-}" ]; then
    if [ "${CALLER_TAG_SPECIFIED:-false}" = "false" ]; then
      ...
      if [ -n "${PRE_EXISTING_IMAGE_ID:-}" ] && [ "${PRE_EXISTING_IMAGE_ID}" = "${CREATED_IMAGE_ID}" ]; then
...
DOCKERFILE_LINT
  ...
  PRE_EXISTING_IMAGE_ID="$(podman images -q --no-trunc "${DERIVED_TAG}" 2>/dev/null || true)"
